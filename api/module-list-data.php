<?php
/**
 * Module List Tool - Direct Database Access API
 * モジュール一覧ツール専用API
 * ツール名: module-list
 */
...
function fetchDirectTable($pdo, $tableName, $lightweight = false) {
    try {
        ...
        if (!tableExists($pdo, $safeTableName)) {
            error_log("[module-list-data.php] Table '{$safeTableName}' does not exist in database");
            return [];
        }
        ...
        error_log("[module-list-data.php] Fetched " . count($rows) . " rows from table '{$safeTableName}'");
        ...
        if ($filteredCount !== count($rows)) {
            error_log("[module-list-data.php] Filtered " . (count($rows) - $filteredCount) . " empty rows from table '{$safeTableName}'");
        }
        
        return array_values($filteredRows);
    } catch (InvalidArgumentException $e) {
        error_log("[module-list-data.php] Invalid table name error: " . $e->getMessage());
        throw $e;
    } catch (PDOException $e) {
        error_log("[module-list-data.php] Error fetching table '{$tableName}': " . $e->getMessage());
        throw $e;
    }
}
...
// Whitelist of tables for Module List Tool
...
try {
    ...
    foreach($tables_to_fetch as $table) {
        ...
        if (!in_array($table, $all_tables_whitelist)) {
            error_log("[module-list-data.php] SECURITY: Attempted access to non-whitelisted table: '{$table}'");
            continue;
        }
        ...
        try {
            ...
            if (!preg_match('/^[a-zA-Z0-9_-]+$/', $table)) {
                error_log("[module-list-data.php] SECURITY: Invalid characters in table name: '{$table}'");
                continue;
            }
            
            // テーブルの存在確認（データベースから直接確認）
            if (!tableExists($pdo, $table)) {
                error_log("[module-list-data.php] Table '{$table}' does not exist in database");
            ...
            if (empty($rawRows)) {
                error_log("[module-list-data.php] Table '{$table}' returned empty array (no rows or all rows filtered), but schema has " . count($schema) . " columns");
            } else {
                error_log("[module-list-data.php] Successfully loaded " . count($rawRows) . " rows from table '{$table}' with " . count($schema) . " columns");
            }
        } catch (InvalidArgumentException $e) {
            error_log("[module-list-data.php] Error loading table '{$table}': " . $e->getMessage());
            ...
            error_log("[module-list-data.php] Error loading table '{$table}': Code {$errorCode}, Message: {$errorMessage}");
            
            if ($e->getCode() == '42S02') { // SQLSTATE[42S02]: Base table or view not found
                $db[$table] = ['schema' => [], 'data' => []];
                error_log("[module-list-data.php] Table '{$table}' not found, returning empty array");
            } else {
                $db[$table] = ['schema' => [], 'data' => []];
                error_log("[module-list-data.php] Table '{$table}' error (not 42S02), returning empty array");
            }
        } catch (Exception $e) {
            error_log("[module-list-data.php] Unexpected error loading table '{$table}': " . $e->getMessage());
            $db[$table] = ['schema' => [], 'data' => []];
        }
    }
