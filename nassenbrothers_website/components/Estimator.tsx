...
// FIX: Imported the missing `ProductPrice` type.
    ProductPrice
} from '../types';
...
const PrintDesignForm: React.FC<PrintDesignFormProps> = ({ design, specialInkOptions, availableLocations, productsInOrder, pricingData, isPrivilegedMode, printSizes, existingDesigns, uiText, onSave, onCancel }) => {
  ...
  const addSpecialInk = () => {
    // FIX: Typed 'opt' to resolve property access on 'unknown' error.
    const availableInkTypes = specialInkOptions
        .map((opt: SpecialInkOption) => opt.type)
...
const ProductSelectionModal: React.FC<ProductSelectionModalProps> = ({ products, tags, categories, colorPalettes, sizeData, stockData, initialProductId, allowedBrand, isPrivilegedMode, isPartnerMode, partnerInfo, uiText, onAddItem, onClose }) => {
    ...
    useEffect(() => {
        ...
        const initialProduct = products.find((p: Product) => p.id === initialProductId);
        if (initialProduct) {
    ...
    const searchResults = useMemo(() => {
        ...
        return products
            .filter((p: Product) => isPrivilegedMode || !allowedBrand || p.brand === allowedBrand)
            .filter((p: Product) => {
    ...
    const selectedProduct = useMemo(() => products.find((p: Product) => p.id === selectedProductId), [selectedProductId, products]);
    ...
    const allProductColors = useMemo(() => {
        ...
        // FIX: Typed 'color' to resolve property access on 'unknown' error.
        return selectedProduct.colors.map(colorCode => allColorsInPalette.find((color: BrandColor) => color.code === colorCode)).filter((color): color is BrandColor => color !== undefined);
    ...
    const availableColors = useMemo(() => {
        if (!selectedProduct) return [];
        // FIX: Typed 'color' to resolve property access on 'unknown' error.
        return allProductColors.map((color: BrandColor) => {
            let isInStock = true;
            if (selectedProduct.brand === 'United Athle' || selectedProduct.brand === 'Print Star') {
                const colorType = color.type;
                const availableSizesForColor = selectedProduct.prices.filter((p: ProductPrice) => p.color === colorType).map(p => p.size);
    ...
    const availableSizes = useMemo(() => {
        if (!selectedProduct) return [];
        // FIX: Typed 'c' to resolve property access on 'unknown' error.
        const colorInfo = Object.values(colorPalette).find((c: BrandColor) => c.name === currentColor);
        const colorType = colorInfo ? colorInfo.type : 'カラー';
        const sizesForColorType = selectedProduct.prices.filter((p: ProductPrice) => p.color === colorType).map(p => p.size);
    ...
    const brands = useMemo(() => {
        ...
        if (isPrivilegedMode || !allowedBrand) {
            return allBrands;
        }
        return allBrands.filter(b => b === allowedBrand);
    }, [products, allowedBrand, isPrivilegedMode]);

    const sortedCategories = useMemo(() => {
        return [...categories].sort((a, b) => (a.sort_order || 99) - (b.sort_order || 99));
    }, [categories]);

    const productsByBrandAndCategory = useMemo(() => {
        const map = new Map<string, Map<string, Product[]>>();
        for (const product of products) {
            if (!map.has(product.brand)) {
                map.set(product.brand, new Map<string, Product[]>());
            }
            const brandMap = map.get(product.brand)!;
            if (!brandMap.has(product.categoryId)) {
                brandMap.set(product.categoryId, []);
            }
            brandMap.get(product.categoryId)!.push(product);
        }
        for (const brandMap of map.values()) {
            for (const productList of brandMap.values()) {
    ...
    const handleAddItemClick = useCallback(() => {
        ...
        const colorInfo = Object.values(colorPalette).find((c: BrandColor) => c.name === currentColor);
        if (!colorInfo) return;
        // FIX: Typed 'p' to resolve property access on 'unknown' error.
        const priceInfo = selectedProduct.prices.find((p: ProductPrice) => p.color === colorInfo.type && p.size === currentSize);
        if (!priceInfo) return;
                    ...
                    <div className="overflow-x-auto border border-border-default">
                        <table className="w-full text-sm text-left min-w-[700px]">
                            <thead className="bg-background-subtle">
                                <tr>
                                    <th className="sticky left-0 bg-background-subtle p-2 font-semibold text-text-secondary z-10 border-r w-32">ブランド</th>
                                    {sortedCategories.map(cat => (
                                        <th key={cat.categoryId} className="p-2 font-semibold text-text-secondary text-center w-48">
                                            {cat.categoryName}
                                        </th>
                                    ))}
                                </tr>
                            ...
                            <tbody>
                                {brands.map(brand => (
                                    <tr key={brand} className="border-t">
                                        <th className="sticky left-0 bg-surface p-2 z-10 border-r w-32 font-bold" scope="row">
                                            {brand}
                                        </th>
                                        {sortedCategories.map(cat => {
                                            const cellProducts = productsByBrandAndCategory.get(brand)?.get(cat.categoryId) || [];
                                            const isSelectedInThisCell = cellProducts.some(p => p.id === selectedProductId);
                                            return (
                                                <td key={cat.categoryId} className="p-1 border-l">
                                                    {cellProducts.length > 0 ? (
                                                        <select
                                                            value={isSelectedInThisCell ? selectedProductId : ''}
                                                            onChange={handleProductChange}
                                                            className="w-full p-1 border border-border-default text-xs"
                                                        >
                                                            <option value="">{cellProducts.length}商品</option>
                                                            {cellProducts.map(p => (
...
export const Estimator: React.FC<EstimatorProps> = ({ appData, initialSearchQuery = '', onNavigateHome, onNavigateToPrivacyPolicy, fromPage }) => {
  ...
  const [isPrintableEstimateVisible, setIsPrintableEstimateVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    if (initialSearchQuery) {
      const productToSelect = allProducts.find((p: Product) => `${p.code} - ${p.name} ${p.variantName || ''}`.trim() === initialSearchQuery);
      if (productToSelect) {
  ...
  const availableLocationsForNewDesign = useMemo(() => {
      ...
      const categoryIdsInOrder = [...new Set(orderDetails.map(item => {
          return allProducts.find((p: Product) => p.id === item.productId)?.categoryId;
  ...
  const productsInOrder = useMemo(() => {
    ...
    return productIds.map(id => allProducts.find((p: Product) => p.id === id)).filter((p): p is Product => !!p);
  }, [orderDetails, allProducts]);
  
  const brandsInOrder = useMemo(() => {
      return [...new Set(orderDetails.map(item => {
          return allProducts.find((p: Product) => p.id === item.productId)?.brand;
    ...
    <>
      ...
      {isPrintableEstimateVisible && (
        <PrintableEstimate
            ...
            uiText={uiText}
            onClose={() => setIsPrintableEstimateVisible(false)}
        />
      )}
