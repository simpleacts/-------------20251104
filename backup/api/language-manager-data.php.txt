<?php
/**
 * Language Manager Tool - Direct Database Access API
 * 言語管理ツール専用API
 * ツール名: language-manager
 */
...
function fetchDirectTable($pdo, $tableName, $lightweight = false) {
    try {
        ...
        if (!tableExists($pdo, $safeTableName)) {
            error_log("[language-manager-data.php] Table '{$safeTableName}' does not exist in database");
            return [];
        }
        ...
        error_log("[language-manager-data.php] Fetched " . count($rows) . " rows from table '{$safeTableName}'");
        ...
        if ($filteredCount !== count($rows)) {
            error_log("[language-manager-data.php] Filtered " . (count($rows) - $filteredCount) . " empty rows from table '{$safeTableName}'");
        }
        
        return array_values($filteredRows);
    } catch (InvalidArgumentException $e) {
        error_log("[language-manager-data.php] Invalid table name error: " . $e->getMessage());
        throw $e;
    } catch (PDOException $e) {
        error_log("[language-manager-data.php] Error fetching table '{$tableName}': " . $e->getMessage());
        throw $e;
    }
}
...
// Whitelist of tables for Language Manager Tool
// Allow language_settings and all language_settings_* tables
// Whitelist of tables for Language Manager Tool
...
// Function to check if a table name is allowed for Language Manager
function isAllowedLanguageTable($tableName) {
    // Explicit whitelist
    $explicitWhitelist = ['language_settings', 'languages'];
    if (in_array($tableName, $explicitWhitelist)) {
        return true;
    }
    
    // Allow language_settings_* pattern (with validation)
    if (strpos($tableName, 'language_settings_') === 0) {
        // Validate: only alphanumeric, underscores, and hyphens after prefix
        $suffix = substr($tableName, strlen('language_settings_'));
...
try {
    ...
    foreach($tables_to_fetch as $table) {
        ...
        // Security: Validate table name - only allow language_settings tables
        if (!isAllowedLanguageTable($table)) {
            error_log("[language-manager-data.php] SECURITY: Attempted access to non-allowed table: '{$table}'");
            continue;
        }
        ...
        try {
            ...
            if (!preg_match('/^[a-zA-Z0-9_-]+$/', $table)) {
                error_log("[language-manager-data.php] SECURITY: Invalid characters in table name: '{$table}'");
                continue;
            }
            
            // テーブルの存在確認（データベースから直接確認）
            if (!tableExists($pdo, $table)) {
                error_log("[language-manager-data.php] Table '{$table}' does not exist in database");
            ...
            if (empty($rawRows)) {
                error_log("[language-manager-data.php] Table '{$table}' returned empty array (no rows or all rows filtered), but schema has " . count($schema) . " columns");
            } else {
                error_log("[language-manager-data.php] Successfully loaded " . count($rawRows) . " rows from table '{$table}' with " . count($schema) . " columns");
            }
        } catch (InvalidArgumentException $e) {
            error_log("[language-manager-data.php] Error loading table '{$table}': " . $e->getMessage());
            ...
            error_log("[language-manager-data.php] Error loading table '{$table}': Code {$errorCode}, Message: {$errorMessage}");
            
            if ($e->getCode() == '42S02') { // SQLSTATE[42S02]: Base table or view not found
                $db[$table] = ['schema' => [], 'data' => []];
                error_log("[language-manager-data.php] Table '{$table}' not found, returning empty array");
            } else {
                $db[$table] = ['schema' => [], 'data' => []];
                error_log("[language-manager-data.php] Table '{$table}' error (not 42S02), returning empty array");
            }
        } catch (Exception $e) {
            error_log("[language-manager-data.php] Unexpected error loading table '{$table}': " . $e->getMessage());
            $db[$table] = ['schema' => [], 'data' => []];
        }
    }
