<?php
/**
 * PHP Info Viewer Tool - Direct Database Access API
 * PHP情報ビューアツール専用API
 * ツール名: php-info-viewer
 */
...
function fetchDirectTable($pdo, $tableName, $lightweight = false) {
    try {
        ...
        if (!tableExists($pdo, $safeTableName)) {
            error_log("[php-info-viewer-data.php] Table '{$safeTableName}' does not exist in database");
            return [];
        }
        
        $stmt = $pdo->query("SELECT * FROM `{$safeTableName}`");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        error_log("[php-info-viewer-data.php] Fetched " . count($rows) . " rows from table '{$safeTableName}'");
        ...
        error_log("[php-info-viewer-data.php] Invalid table name error: " . $e->getMessage());
        throw $e;
    } catch (PDOException $e) {
        error_log("[php-info-viewer-data.php] Error fetching table '{$tableName}': " . $e->getMessage());
        throw $e;
    }
}
...
// Whitelist of tables for PHP Info Viewer Tool
// 目的: ツールごとのアクセス制限（セキュリティ）
// 注意: tableExists()は「テーブルの存在確認」、ホワイトリストは「ツールがアクセス可能なテーブル」を制限する
// 両方の役割が異なるため、両方必要
$all_tables_whitelist = [
    'language_settings_common', 'language_settings_php_info_viewer'
];
...
try {
    ...
    foreach($tables_to_fetch as $table) {
        ...
        if (!in_array($table, $all_tables_whitelist)) {
            error_log("[php-info-viewer-data.php] SECURITY: Attempted access to non-whitelisted table: '{$table}'");
            continue;
        }
        
        try {
            if (!preg_match('/^[a-zA-Z0-9_-]+$/', $table)) {
                error_log("[php-info-viewer-data.php] SECURITY: Invalid characters in table name: '{$table}'");
                continue;
            }
            
            // テーブルの存在確認（データベースから直接確認）
            if (!tableExists($pdo, $table)) {
                error_log("[php-info-viewer-data.php] Table '{$table}' does not exist in database");
            ...
            if (empty($rawRows)) {
                error_log("[php-info-viewer-data.php] Table '{$table}' returned empty array, but schema has " . count($schema) . " columns");
            } else {
                error_log("[php-info-viewer-data.php] Successfully loaded " . count($rawRows) . " rows from table '{$table}' with " . count($schema) . " columns");
            }
        } catch (Exception $e) {
            error_log("[php-info-viewer-data.php] Error loading table '{$table}': " . $e->getMessage());
            $db[$table] = ['schema' => [], 'data' => []];
        }
    }
