<?php
/**
 * Shipping Logic Tool - Direct Database Access API
 * 配送ロジックツール専用API
 * ツール名: shipping-logic-tool
 */
...
function fetchDirectTable($pdo, $tableName, $lightweight = false) {
    try {
        ...
        if (!tableExists($pdo, $safeTableName)) {
            error_log("[shipping-logic-tool-data.php] Table '{$safeTableName}' does not exist in database");
            return [];
        }
        
        $stmt = $pdo->query("SELECT * FROM `{$safeTableName}`");
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
        error_log("[shipping-logic-tool-data.php] Fetched " . count($rows) . " rows from table '{$safeTableName}'");
        ...
        error_log("[shipping-logic-tool-data.php] Invalid table name error: " . $e->getMessage());
        throw $e;
    } catch (PDOException $e) {
        error_log("[shipping-logic-tool-data.php] Error fetching table '{$tableName}': " . $e->getMessage());
        throw $e;
    }
}
...
// Whitelist of tables for Shipping Logic Tool (same as estimator)
$all_tables_whitelist = [
    ...
    // 言語設定
    'language_settings_common', 'language_settings_estimator_settings', 'language_settings_shipping_logic_tool'
];
...
try {
    ...
    foreach($tables_to_fetch as $table) {
        ...
        if (!in_array($table, $all_tables_whitelist)) {
            error_log("[shipping-logic-tool-data.php] SECURITY: Attempted access to non-whitelisted table: '{$table}'");
            continue;
        }
        
        try {
            if (!preg_match('/^[a-zA-Z0-9_-]+$/', $table)) {
                error_log("[shipping-logic-tool-data.php] SECURITY: Invalid characters in table name: '{$table}'");
                continue;
            }
            
            // テーブルの存在確認（データベースから直接確認）
            if (!tableExists($pdo, $table)) {
                error_log("[shipping-logic-tool-data.php] Table '{$table}' does not exist in database");
            ...
            if (empty($rawRows)) {
                error_log("[shipping-logic-tool-data.php] Table '{$table}' returned empty array, but schema has " . count($schema) . " columns");
            } else {
                error_log("[shipping-logic-tool-data.php] Successfully loaded " . count($rawRows) . " rows from table '{$table}' with " . count($schema) . " columns");
            }
        } catch (Exception $e) {
            error_log("[shipping-logic-tool-data.php] Error loading table '{$table}': " . $e->getMessage());
            $db[$table] = ['schema' => [], 'data' => []];
        }
    }
